...
set fetch 	"FabScalarParam.v SimpleScalar_ISA.v RAS.v BTB.v SelectInst.v \
		 ...
		 Fetch2Decode.v CtrlQueue.v L1ICache.v"
...
analyze -library WORK -format verilog $misc
analyze -library WORK -format verilog $fetch
analyze -library WORK -format verilog $decode
analyze -library WORK -format verilog $rename
analyze -library WORK -format verilog $dispatch
analyze -library WORK -format verilog $issueq
analyze -library WORK -format verilog $execute
analyze -library WORK -format verilog $lsq
analyze -library WORK -format verilog $writebk
analyze -library WORK -format verilog $retire
analyze -library WORK -format verilog $pmem
analyze -library WORK -format verilog $top

elaborate FABSCALAR -architecture verilog -library WORK
